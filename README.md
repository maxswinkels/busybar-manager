...
- **Apps** (`/apps`) — every discovered app with an on/off switch, a variation picker, restart, live logs, and a badge showing who owns the screen. Also where you build **scrollers**: named cycles that page through several apps.
- **Schedule** (`/schedule`) — a weekly repeating timetable: pin an app + variation, or a whole scroller, to a time window on a weekday.
...
A **scroller** is a named cycle of apps: each one holds the bar for a few seconds and then hands over to the next, wrapping at the end. That way every app can stay simple and show a single piece of information, while the bar still pages through all of them.

Build one with **+ Add Scroller** on the Apps tab. In the popup you set a name and a **base duration** (seconds per app), then add as many apps as you like — each with its own variation, moved up and down into the order you want, and optionally with its own duration overriding the base value. Saved scrollers appear in the Apps list above the apps themselves, with a switch to run the cycle, Edit, and Delete.

- Running a scroller does **not** flip the on/off switch of the apps it cycles. While a step is on the bar, that app's row shows a `SCROLLER · <variation>` chip; handing over stops it again and clears its frame.
- An app you switched on yourself keeps running alongside — a scroller never stops it.
- Changing a scroller's apps or durations restarts its cycle from the first step; renaming it doesn't disturb what is on the bar.
- Steps whose app has been removed are skipped, and a scroller can also be driven by the Schedule tab (see below) instead of its own switch.
...
A slot pins one app and one of its variations — or one whole **scroller** — to a `[start, end)` window on **one or more weekdays**; "Mon–Fri 08:00–10:00" is one slot, not five. Slots never overlap on a day they share, so at most one thing is ever scheduled at a time, and a gap simply means nothing is scheduled then.
...
- A slot set to a **scroller** runs that whole cycle for the length of the window, without switching the scroller on itself; when the window ends the cycle stops and the last app is cleared off the bar.
...
| `PUT /api/_manager/schedule` · `POST …/schedule/slots` · `PUT`/`DELETE …/schedule/slots/<id>` | Weekly timetable: master switch + slot CRUD |
| `GET /api/_manager/scrollers` · `POST …` · `PUT`/`DELETE …/scrollers/<id>` · `POST …/<id>/enable`·`/disable` | Scrollers: cycle CRUD + their own switch |
